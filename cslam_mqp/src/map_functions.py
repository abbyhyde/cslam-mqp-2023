...
def dilate_map(mapdata, padding):
    ...
    expanded_walls = scipy.ndimage.binary_dilation(np.greater_equal(mapdata.data, 5).reshape((new_map.info.height, new_map.info.width))*1, iterations=int(padding)).reshape(new_map.info.height*new_map.info.width)
    new_map.data = np.add(np.multiply(mapdata.data,np.logical_not(expanded_walls)), expanded_walls*100).tolist()
