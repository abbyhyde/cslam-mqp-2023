...
#settings params from params.py
  ...
  while(sum(val for val in weight_dict.values()) > 0):
    ...
    m.addConstr(vertexes.prod(weight_dict) <= robot_memory)
    m.addConstr(vertexes[0] == 1)
    m.addConstrs(vertexes[v]*edges.sum(v,'*') == edges.sum(v,'*') for v in vertex_labels.keys())
    m.addConstrs(vertexes[v]*edges.sum(v,'*') >= vertexes[v]      for v in vertex_labels.keys())
